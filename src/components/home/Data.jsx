...
const Data = () => {
  ...
  const [index, setIndex] = useState(0); // To track the current title
  const [displayedText, setDisplayedText] = useState(""); // For the letter-by-letter effect
  const [letterIndex, setLetterIndex] = useState(0); // To track the current letter

  useEffect(() => {
    if (letterIndex < titles[index].length) {
      const timeout = setTimeout(() => {
        setDisplayedText((prev) => prev + titles[index][letterIndex]);
        setLetterIndex((prev) => prev + 1);
      }, 150); // Adjust the speed of each letter appearing
      return () => clearTimeout(timeout);
    } else {
      // Once the title is fully displayed, wait before switching to the next title
      const pauseTimeout = setTimeout(() => {
        setLetterIndex(0);
        setDisplayedText("");
        setIndex((prevIndex) => (prevIndex + 1) % titles.length);
      }, 1000); // Adjust the pause duration
  ...
  return (
    <div className="home__data">
      ...
      <h3 className="title-container"> I am a...<h3 > {displayedText}</h3></h3>
     
      <p className="home__description" >
      I'm a passionate Web Developer. I enjoy transforming complex ideas into smooth, user-friendly experiences that make an impact.
      </p>
